...
int malloc_last_allocated_page_index;

void malloc_bitmap_init()
{
    malloc_last_allocated_page_index = -1;
    for (uint16_t i = 0; i < 24576; i++)
        *(uint32_t*)((uint32_t)&malloc_pages_bitmap[0] + 4 * (uint32_t)i) = 0;
}

bool malloc_bitmap_get_page(uint32_t page)
{
    if (page >= malloc_last_allocated_page_index) return 1;
...
void malloc_bitmap_set_page(uint32_t page, bool state)
{
    ...
    uint8_t bit = page & 0b111;
    if (state)  malloc_pages_bitmap[byte] |= (1 << bit);
    else        malloc_pages_bitmap[byte] &= ~(1 << bit);

    if (state && malloc_last_allocated_page_index < page) malloc_last_allocated_page_index = page;
    if (!state && malloc_last_allocated_page_index == page)
    {
