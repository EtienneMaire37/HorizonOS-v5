...
physical_address_t vas_create_empty()
{
    ...
    physical_add_page_table(cr3, 0, first_mb_pt_address, PAGING_SUPERVISOR_LEVEL, true);
    ...
    physical_init_page_table(pt_address);
    physical_add_page_table(cr3, 767, pt_address, PAGING_SUPERVISOR_LEVEL, true);
    ...
    for (int i = stack_page_index_start; i <= stack_page_index_end; i++)
        physical_set_page(pt_address, i, pfa_allocate_physical_page(), PAGING_SUPERVISOR_LEVEL, true);
