...
// {
//     #ifdef LOG_SYSCALLS
//     LOG(DEBUG, "Task \"%s\" (pid = %d) sent system call %llu", __CURRENT_TASK.name, __CURRENT_TASK.pid, registers->rax);
//     #endif

//     switch (registers->rax) // !! some of these path resolution is handled in libc
//     {
//     case SYSCALL_EXIT:     // * exit | exit_code = $rbx |
//         LOG(WARNING, "Task \"%s\" (pid = %d) exited with return code %d", __CURRENT_TASK.name, __CURRENT_TASK.pid, registers->rbx);
//         lock_task_queue();
//         __CURRENT_TASK.is_dead = true;
//         __CURRENT_TASK.return_value = registers->rbx & 0xff;
//         unlock_task_queue();
//         switch_task();
//         break;
//     case SYSCALL_TIME:      // * time || $rax = time
//         registers->rax = ktime(NULL);
//         break;
...
//             break;
//         }
//         startup_data_struct_t data = startup_data_init_from_argv((const char**)registers->rcx, (char**)registers->rdx, (char*)registers->rsi);
//         lock_task_queue();
//         if (!multitasking_add_task_from_vfs((char*)registers->rbx, (char*)registers->rbx, 3, false, &data))
//         {
//             unlock_task_queue();
...
//         registers->rbx = (uint32_t)vfs_readdir(dirent_entry, dirp);
//         registers->rax = errno;
//         break;
//     }
...
//     case SYSCALL_SET_KB_LAYOUT:
//         if (__CURRENT_TASK.ring == 0 && registers->rbx >= 1 && registers->rbx <= NUM_KB_LAYOUTS)
//         {
//             current_keyboard_layout = keyboard_layouts[registers->rbx - 1];
//             registers->rax = 1;
//         }
...
//             uint32_t pde = read_physical_address_4b(__CURRENT_TASK.cr3 + 4 * (registers->rbx >> 22));
...
//                                         true);
//             }
//             uint32_t pte = read_physical_address_4b(pt_address + 4 * ((registers->rbx >> 12) & 0x3ff));
//             if (!(pte & 1))
//             {
...
//                 #ifdef USE_IVLPG
//                 uint32_t* recursive_paging_pte = (uint32_t*)(((uint32_t)4 * 1024 * 1024 * 1023) | (4 * ((registers->rbx >> 22) * 1024 + ((registers->rbx >> 12) & 0x3ff))));
...
//                 invlpg(4096 * (uint32_t)((registers->rbx >> 22) * 1024 + ((registers->rbx >> 12) & 0x3ff)));
//                 #else
...
//                 physical_remove_page(pt_address, ((registers->rbx >> 12) & 0x3ff));
